...
var assembly = {
  ...
  /**
   * Meta data for materials, grouped by "collection" (sub-directory);
   * contains name and sub-items
   * @type {Object}
   */
  materials: {},
...
var getName = function (filePath, preserveNumbers) {
  ...
  var name = path.basename(filePath,
    path.extname(filePath)).replace(/\s/g, '-');
...
var handleError = function (e) {
  ...
  if (options.logErrors) {
    console.error(chalk.bold.red('Error (fabricator-assemble): ' +
      e.message + '\n'), e.stack);
    exit = false;
  }
...
/**
 * Build the template context by merging context-specific data w/ assembly data
 * @param  {Object} data
 * @return {Object}
 */
var buildContext = function (data, hash) {
  ...
  return _.assign({}, data, assembly.data, assembly.materialData,
    materials, views, docs, hash);
};
...
var parseMaterials = function () {
  ...
  files.forEach(function (file) {
    var pathName = path.normalize(path.dirname(file));
    var parent = getName(pathName.split(path.sep).slice(-2, -1)[0], true);
    var collection = getName(pathName.split(path.sep).pop(), true);
    var isSubCollection = (dirs.indexOf(parent) > -1);

    // get the material base dir for stubbing out the base object
    // for each category (e.g. component, structure)
...
var assemble = function () {
  ...
  files.forEach(function (file) {
    ...
    var filePath = path.normalize(path.join(options.dest, collection, path.basename(file)));
